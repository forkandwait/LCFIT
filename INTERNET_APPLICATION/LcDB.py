...
def test_pause(dbcon):
	...
	(backend_pid,) = res
	syslog.syslog("Pausing before grabbing a cursor. Backend pid: %s." % backend_pid)
	signal.pause()
	syslog.syslog("Un-pausing.")
...
def try_execute(dbcon, sql, data=None, fetch_N='all', tries_N=3):
	...
	while tries_N >= 0:
		try:
			...
			syslog.syslog("Error trying to execute query: \"%s\", \"%s\"." % (sql, e))
			tries_N -= 1
			...
			syslog.syslog("Successfully reconnected, re-executed cursor.")
		except:
			raise
	raise 
