...
class LcRegistrationForm:
	...
	def __call__(self, req):
		...
		syslog.syslog('LcRegistrationForm:  __call__')
...
class LcRegistrationProcess:
	...
	def __call__(self, req):
		...
		# Log
		syslog.syslog('LcRegistrationProcess:  __call__')
		...
		try:
			...
			syslog.syslog('Bad Registration request: %s.' % pprint.pformat(e))
...
class LcLoginForm:
	...
	def __call__(self, req):
		...
		# Log
		syslog.syslog('LcLoginForm:  __call__')
		...
		#formTemplateComp = Template.Template.compile(file=self.formTemplate, searchList = searchList)
		formTemplate = Template.Template(file=self.formTemplate, searchList = searchList)
	...
	def __call__(self, req):
		...
		if self._hasNecessaryData(req):
			...
			if type(authId) is types.IntType: 
				...
				syslog.syslog('LcLoginProcess:  __call__.  Successfully logged in.  Auth id: %s, username: %s.' \
		...
		# Log
		syslog.syslog('LcLoginProcess:  __call__.  Unsuccessful login.')
...
class LcLogout:
	...
	def __call__(self, req):
		...
		if req.session.has_key(LARRY_SESSION_KEY):
			...
			syslog.syslog('LcLogout:  __call__.  Logging out session key: %s' % req.session[LARRY_SESSION_KEY])
		req.session.invalidate()
		syslog.syslog('LcLogout:  __call__.  Logging out unkown session.')
		util.redirect(req, self.redirectTarget)
...
class LcIndex(LcPage):
	...
	def __call__(self, req):
		syslog.syslog('LcIndex:  __call__.')
...
class LcProcess(LcPage):
	...
	def __call__(self, req):
		...
		try:
			...
			syslog.syslog('LcProcess:  __call__.  Object ID: %s.  Datapath: %s. Class: %s' % \
...
class LcDisplay(LcPage):
	...
	def __call__(self, req):
		try:
			...
			syslog.syslog("LcDisplay: __call__.  Object ID:  %s" % objId)
			...
			syslog.syslog("LcDisplay: __call__.  Error.")
			util.redirect(req, LcSessionExceptionRedirect)
		except LcDataException:
			syslog.syslog("LcDisplay: __call__.  Error.  Object ID: %s" % objId)
...
class LcList(LcPage):
	...
	def __call__(self, req):
		try:
			...
			syslog.syslog("LcList: __call__.  User: %s." % currentUser)
			...
			syslog.syslog("LcList: __call__.  Error.")
			util.redirect(req, LcSessionExceptionRedirect)
		except (LcDataException), e:
			syslog.syslog("LcList: __call__.  Error.")
	...
	def __call__(self, req):
		try:
			...
			if currentUser != objectOwner:
				...
				syslog.syslog("LcDelete: __call__.  Deleted object ID: %s." % objId)
				util.redirect(req, self.redirectTarget)

		except (LcSessionException,):
			syslog.syslog("LcDelete: __call__.  Error.")
			util.redirect(req, LcSessionExceptionRedirect)
...
class LcDumpText(LcPage):
	...
	def __call__(self, req):
		try:
			...
			self._postCondition(req)
			syslog.syslog("LcDumpText: __call__.  ObjectId: %s."  % objectId)
...
class LcError:
	...
	def __call__(self, req, **kwargs):
		...
		syslog.syslog("LcError: __call__.")
